...
let leads: Lead[] = [
  {
    ...
    priceMax: 3500,
    leadType: 'rent',
    notes: 'Wants near the Q train. No walk-up.',
    source: 'Manual',
  ...
  },
  {
    ...
    priceMax: 5200,
    leadType: 'rent',
    notes: '2BR, washer/dryer, pets ok.',
    source: 'Meta',
...
export function mask(l: Lead): Lead {
  ...
  return { ...rest }; // hide PII in unclaimed view
}

export function getUnclaimed(): Lead[] {
  return leads.filter(l => l.status === 'unclaimed').map(mask);
}

export function getMine(userId: string): Lead[] {
  return leads.filter(l => l.claimedByUserId === userId);
}
...
export function claimLead(userId: string, leadId: string):
  ...
  if (leads[i].status !== 'unclaimed') return { ok: false, reason: 'already_claimed' };
  leads[i] = {
    ...leads[i],
    status: 'claimed',
    claimedByUserId: userId,
    claimedAt: new Date().toISOString(),
    // When you wire a real DB/GHL, populate actual PII here
    phone: '(212) 555-0199',
